...
Deno.serve(async (req) => {
  ...
  const cardletTable = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid rgba(255,255,255,0.1);border-bottom:1px solid rgba(255,255,255,0.1);margin-top:16px;">
      ...
      ${validUntil ? row('صلاحية البطاقة', `صالحة حتى ${validUntil} (لحين استكمال إجراءات التثبيت)`) : ''}
    </table>
  `;
  ...
  if (customBody) {
    ...
    if (isTransferred) {
      text = [
        ...
        validUntil ? `• صلاحية البطاقة: حتى ${validUntil} (لحين استكمال إجراءات التثبيت والاعتماد النهائي للعضوية)` : '',
        '',
      ...
      text = [
        ...
        validUntil ? `صلاحية البطاقة: صالحة حتى ${validUntil} (لحين استكمال إجراءات التثبيت والاعتماد النهائي للعضوية)` : '',
        '',
