...
import { CreditCard, Edit3, ShieldCheck } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {colleges.map((col) => {
        const isNamedCoordinator = Boolean(
          col.coordinator.name && col.coordinator.name.trim() && col.coordinator.name !== 'ممثلو الكلية في النادي'
        );
        return (
          <div
            key={col.id}
              ...
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5 flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    <span className="text-xs text-gray-400">منسق وممثل الكلية:</span>
                    {isNamedCoordinator && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-cyan-300 bg-cyan-950/70 border border-cyan-500/40 px-1.5 py-0.5 rounded">
                        <ShieldCheck className="w-3 h-3 text-cyan-400" /> مرتبط بالكادر
                      </span>
                    )}
                  </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-white/5">
              {onViewCoordinatorBadge && (
                <button
                  ...
                  className="py-2 px-3 rounded-xl bg-cyan-950/60 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
              ...
              <button
                ...
                className="flex-1 py-2 rounded-xl bg-white/[0.05] hover:bg-cyan-400 hover:text-black text-cyan-300 text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
